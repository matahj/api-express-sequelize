...
////////////////////////////////////////////////
app.post("/terminal", async function (req, res) {
  const { nombre, estado, direccion } = req.body;

  const documento = await Terminal.create({ nombre, estado, direccion });

  res.send(documento);
});

app.get("/terminal", async function (req, res) {
  documentos = await Terminal.findAll();
  res.send(documentos);
});

app.post("/autobus", async function (req, res) {
  const { matricula, marca, anio, activo, terminaleId } = req.body;

  const documento = await Autobus.create({
    matricula,
    marca,
    anio,
    activo,
...
app.get("/autobus", async function (req, res) {
  documentos = await Autobus.findAll({ include: Terminal });
  res.send(documentos);
});

app.get("/autobus/:id", async function (req, res) {
  const {id} = req.params;
  console.log(id);
  documento = await Autobus.findByPk(id, { include: Terminal });
  res.send(documento);
});
